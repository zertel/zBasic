...
if(strpos(z('host'),'localhost')!==false || strpos(z('host'),'192.168.')!==false){   // Eğer yerel sunucuda isem
	z('con',Array('localhost','root','root','zwork','zw_','urun'));   // Localhost veritabanına bağlan
}
else{   // Eğer yerel sunucuda değil isem
	z('con',Array('localhost','sunucu_kullanici_adi','sunucu_kullanici_sifre','sunucudaki_veritabani_adi','zw_')
...
z('ini','t_low',true);   // Veritabanındaki tablo adları büyük küçük harf içermiyor, tamamı küçük harflerden oluşuyor ise ve sistemde tablo tanımlamalarında büyük harf kullanılmış ise aktif edilmeli. (tablo adlarını her zaman küçük harfle sorguya gönderir. Ör: uyeYorumlari --> uyeyorumlari)

z('lgna','tablo','personel');   // Üyelik girişi için kullanılan üyeler tablosunun adı
z('lgna','alan',Array('kullanici','sifre',true),Array('eposta','sifre',true));   // üye giriş anında kullanılan alanların tanımlanması
